...
const SimplifiedChineseEmbed: React.FC<SimplifiedChineseEmbedProps> = ({ sso = false }) => {
  ...
  useEffect(() => {
    ...
    if (email) {
      ...
      const redirectUrl = `${appDomain}/sso-login?token=${simpleToken}&email=${encodeURIComponent(email)}&name=${encodeURIComponent(name || email.split('@')[0])}&lang=zh-cn`;
    ...
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f9f9f9' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ marginTop: '20px', color: '#4f46e5', fontFamily: 'Arial, sans-serif' }}>正在登录...</p>
        </div>
        <style>{`
          @keyframes spin {
...
function createDirectAccessLink(email: string, language = 'zh-cn') {
  // Create a simple token by encoding the email and timestamp
  const simpleToken = btoa(`${email}:${Date.now()}`);
...
function quickAccess(language: string) {
  const testEmail = prompt("输入电子邮件以访问简体中文评估:", "user@example.com");
  if (testEmail) {
